...
export function WeeklyStats({ repairOrders, height = 200 }) {
  const weeklyData = useMemo(() => {
    ...
    const days = Array.from({ length: 7 }).map((_, index) => {
      ...
      const laborHours = dayOrders.reduce((sum, order) => sum + Number.parseFloat(order.labor_hours || 0), 0)
          ...
          {weeklyData.days.map((day, index) => (
            <div key={index} className="flex flex-col items-center gap-2 w-1/7">
              <div className="relative w-full flex justify-center">
                ...
                {/* Orders bar */}
                <div
                  className="w-4 bg-blue-500 rounded-t-sm absolute left-1/2 transform -translate-x-8"
                  style={{
